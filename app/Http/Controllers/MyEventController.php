...
class MyEventController extends Controller
{
    ...
    {
        $events = \App\Events::where('id_user',Auth::user()->id)->get();
        return view('myevent',['events' => $events]);
    }

    public function delete($id)
    {
        $events = \App\Events::where('id',$id)->delete();
        return redirect('/myevent');
    }
    public function edit($id)
    {
        $events = \App\Events::where('id',$id)->get();
        return view('edit')->with('events',$events);
    }
    public function update(Request $request)
    {
        $event = \App\Events::find($request->input('id'));
